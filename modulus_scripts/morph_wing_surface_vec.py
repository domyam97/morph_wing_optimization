...
def load_data(cfg, res_path, csv_name, n_train=50, n_test=10, n_params = 4):
    if os.path.isfile("/tmp/data/data.npz"):
        ...
        for i in range(n_train):
            ...
            tau_train = np.concatenate((tau_train,tau*(time_scale**2)/(len_scale**2)), axis=0)
        ...
        print("tau", np.max(tau_train),np.min(tau_train))
        ...
        for i in range(n_test):
            ...
            #V_test  = np.concatenate((V_test,np.ones((points.shape[0],1))*csv_data[-(i+1)]["V"]),axis = 0)
            ...
            # Get Outputs
            tau_test = np.concatenate((tau_test,tau*(time_scale**2)/(len_scale**2)), axis=0)
        ...
        print("tau", np.max(tau_test),np.min(tau_test))
...
@modulus.sym.main(config_path="conf", config_name="conf_surf_vec.yaml")
def run(cfg: ModulusConfig):
    ...
    csv_name = 'morph-wing_dataset_no_def.csv'
    ...
    cfg.initialization_network_dir = os.path.expandvars("${SCRATCH}/modulus/outputs/morph-wing_surf_vec")
    cfg.network_dir = os.path.expandvars("${SCRATCH}/modulus/outputs/morph-wing_surf_vec")
...
def solve_nn(cfg, domain, nodes):
    ...
    if cfg.run_mode == "eval":
        cfg.initialization_network_dir = os.path.expandvars("${HOME}/fly-by-feel/modulus_results/morph-wing_surf_vec")
        cfg.network_dir = os.path.expandvars("${SCRATCH}/modulus/outputs/morph-wing_surf_vec_results")
        sp = {}
        V = cfg.custom.Vinf
