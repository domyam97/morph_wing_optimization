...
def load_data(res_path, csv_name, n_train=50, n_test=10):
    ...
    for i in range(n_train):
        ...
        x_train = np.concatenate((x_train, points[:,0].reshape((points.shape[0],1))), axis = 0)
        y_train = np.concatenate((y_train, points[:,1].reshape((points.shape[0],1))), axis = 0)
        z_train = np.concatenate((z_train, points[:,2].reshape((points.shape[0],1))), axis = 0)
        # Set Input Vals
        params = np.array([csv_data[i]["V"],csv_data[i]["alpha"],csv_data[i]["d1"],csv_data[i]["d2"]])
        ...
        # Get Outputs
        u_train = np.concatenate((u_train,vtk_obj.get_array("U")[:,0].reshape((points.shape[0],1))), axis=0)
        v_train = np.concatenate((v_train,vtk_obj.get_array("U")[:,1].reshape((points.shape[0],1))), axis=0)
        w_train = np.concatenate((w_train,vtk_obj.get_array("U")[:,2].reshape((points.shape[0],1))), axis=0)
        p_train = np.concatenate((p_train,vtk_obj.get_array("p")-101e3), axis=0)    
        nuT_train = np.concatenate((nuT_train,vtk_obj.get_array("nuTilda")),axis = 0)
    ...
    for i in range(n_test):
        ...
        # Get Points
        points = vtk_obj.get_points()
        x_test = np.concatenate((x_test, points[:,0].reshape((points.shape[0],1))), axis = 0)
        y_test = np.concatenate((y_test, points[:,1].reshape((points.shape[0],1))), axis = 0)
        z_test = np.concatenate((z_test, points[:,2].reshape((points.shape[0],1))), axis = 0)
        # Set Input Vals
        params = np.array([csv_data[-(i+1)]["V"],csv_data[-(i+1)]["alpha"],csv_data[-(i+1)]["d1"],csv_data[-(i+1)]["d2"]])
        ...
        # Get Outputs
        u_test = np.concatenate((u_test,vtk_obj.get_array("U")[:,0].reshape((points.shape[0],1))), axis=0)
        v_test = np.concatenate((v_test,vtk_obj.get_array("U")[:,1].reshape((points.shape[0],1))), axis=0)
        w_test = np.concatenate((w_test,vtk_obj.get_array("U")[:,2].reshape((points.shape[0],1))), axis=0)
        p_test = np.concatenate((p_test,vtk_obj.get_array("p")-101e3), axis=0)    
        nuT_test = np.concatenate((nuT_test,vtk_obj.get_array("nuTilda")), axis=0)
...
@modulus.sym.main(config_path="conf", config_name="conf.yaml")
def run(cfg: ModulusConfig) -> None:
    ...
    deeponet = DeepONetArch(
        output_keys=[Key("u"),Key("v"),Key("w"),Key("p"), Key("nu")],
        branch_net=branch_net,
        trunk_net=trunk_net,
    )

    deepo_nodes = [deeponet.make_node("deepo")]
    ...
    csv_name = 'morph-wing_dataset.csv'
    res_path = os.path.expanduser('~/fly-by-feel/morph-wing/vtk_res')
    data = load_data(res_path, csv_name, 60,40)
    
    # [constraint]
    # Make Domain
    domain = Domain()

    datacon = DeepONetConstraint.from_numpy(
        nodes = nodes,
        invar = {
            "x":data["x_train"],
            "y":data["y_train"],
    ...
    domain.add_constraint(datacon, "data")
